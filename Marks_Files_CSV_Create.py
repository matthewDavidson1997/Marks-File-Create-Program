...
def validate_match(key, **input_prompt_detail) -> str:
    # Get valid pattern and messages from dicts
    regex_pattern = REGEX_PATTERNS[key]
    prompt, repeat = MESSAGE_DICT[key]

    if len(input_prompt_detail):
        input_prompt_detail = input_prompt_detail["input_prompt_detail"] + ": "
    else:
        input_prompt_detail = ""

    while True:
        # Get user input
        user_input = input(prompt + input_prompt_detail).upper()
...
def get_qpvs(candidates: range, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if exemptions_choice == "Y":
        ...
        for module in df['Module Code'].unique():
            module_choice = input(f"{module} exempt? (y/n): ").upper()
            if module_choice == "Y":
    ...
    for module in df['Module Code'].unique():
        ...
        qpv = validate_match(key='QPV', input_prompt_detail=module)
...
def add_candidates_to_df(candidates: range, df: pd.DataFrame,
    ...
    candidate_df["Max_Mark"] = candidate_df["Max_Mark"]
    return candidate_df
